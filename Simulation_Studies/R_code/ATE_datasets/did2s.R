...
calculate_error_metrics <- function(estimated_ATE, accumulated_p_values, true_ATE = 0.5, suffix = "") {
  ...
  filename <- paste0("did2s_ATE_and_PValues", suffix, ".csv")
...
total_PTA_respected <- total_PTA_respected + ifelse(mean(head(abs(out1$estimate / out1$std.error),num_pre_periods)) > 01.96, 1, 0)
