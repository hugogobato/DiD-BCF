...
# Check if the model ran successfully (out1_result is not NULL)
if (!is.null(out1_result)) {
    out1 <- out1_result
} else {
out1 <- att_gt(yname="Y",
tname="time",
...
estimated_ATE[,iteration+1,group] <- out1$att[(4+7*(group-1)):(7+7*(group-1))]
accumulated_p_values[,iteration+1,group] <- out1$sig[(4+7*(group-1)):(7+7*(group-1))]
...
output_filename_excel <- paste0('DoubleML_did_CATE_GATE_PS_and_PValues_', linearity_folder, '.xlsx')
