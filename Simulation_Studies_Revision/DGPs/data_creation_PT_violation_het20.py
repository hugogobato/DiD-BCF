#!/usr/bin/env python3
"""DGP: PT_violation_het20  (workstream PT, canonical DiD (selection on unobservables)).
...
``linearity_degree`` to
``R_code/PT_violation_het20_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/PT_violation_het20_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_PT_violation_het20.py --reps 100 --jobs 8
    python DGPs/data_creation_PT_violation_het20.py --all-N        # sweep scenarios only
"""
