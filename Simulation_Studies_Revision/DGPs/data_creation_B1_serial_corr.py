#!/usr/bin/env python3
"""DGP: B1_serial_corr  (workstream B1, canonical DiD (selection on unobservables)).

AR(1) within-unit serially correlated errors

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/B1_serial_corr_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/B1_serial_corr_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_B1_serial_corr.py --reps 100 --jobs 8
    python DGPs/data_creation_B1_serial_corr.py --all-N        # sweep scenarios only
"""
