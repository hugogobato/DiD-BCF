#!/usr/bin/env python3
"""DGP: PT_violation_g20  (workstream PT, canonical DiD (selection on unobservables)).

treated group on a differential path, slope 0.2 per period: detection power against violation magnitude

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/PT_violation_g20_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/PT_violation_g20_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_PT_violation_g20.py --reps 100 --jobs 8
    python DGPs/data_creation_PT_violation_g20.py --all-N        # sweep scenarios only
"""
