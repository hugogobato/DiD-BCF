#!/usr/bin/env python3
"""DGP: B1_selection_obs  (workstream B1, canonical DiD (selection on unobservables)).

continuity check: selection on observables only

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/B1_selection_obs_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/B1_selection_obs_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_B1_selection_obs.py --reps 100 --jobs 8
    python DGPs/data_creation_B1_selection_obs.py --all-N        # sweep scenarios only
"""
