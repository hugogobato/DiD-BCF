#!/usr/bin/env python3
"""DGP: B1_null  (workstream B1, canonical DiD (selection on unobservables)).

sharp null tau=0: size and coverage under H0

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/B1_null_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/B1_null_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_B1_null.py --reps 100 --jobs 8
    python DGPs/data_creation_B1_null.py --all-N        # sweep scenarios only
"""
