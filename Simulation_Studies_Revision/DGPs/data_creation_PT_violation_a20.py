#!/usr/bin/env python3
"""DGP: PT_violation_a20  (workstream PT, canonical DiD (selection on unobservables)).

the unobserved confounder also drives the slope (lambda=0.2): violation of conditional PT that no adjustment on observables can remove.  Single mechanism check -- see ALPHA_TREND_CHECK for why this is not a magnitude grid

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/PT_violation_a20_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/PT_violation_a20_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_PT_violation_a20.py --reps 100 --jobs 8
    python DGPs/data_creation_PT_violation_a20.py --all-N        # sweep scenarios only
"""
