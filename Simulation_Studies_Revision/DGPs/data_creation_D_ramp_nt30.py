#!/usr/bin/env python3
"""DGP: D_ramp_nt30  (workstream D, staggered adoption (cohort x event-time effects)).

never-treated share 0.30: Goodman-Bacon weight on already-treated comparisons rises as the clean control pool shrinks (effect DGP held fixed)

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/D_ramp_nt30_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/D_ramp_nt30_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_D_ramp_nt30.py --reps 100 --jobs 8
    python DGPs/data_creation_D_ramp_nt30.py --all-N        # sweep scenarios only
"""
