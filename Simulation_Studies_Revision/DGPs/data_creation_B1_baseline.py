#!/usr/bin/env python3
"""DGP: B1_baseline  (workstream B1, canonical DiD (selection on unobservables)).

moderate unit FE, selection on unobservables, iid errors

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/B1_baseline_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/B1_baseline_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_B1_baseline.py --reps 100 --jobs 8
    python DGPs/data_creation_B1_baseline.py --all-N        # sweep scenarios only
"""
