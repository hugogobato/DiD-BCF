#!/usr/bin/env python3
"""DGP: PT_conditional  (workstream PT, canonical DiD (selection on unobservables)).

conditional PT holds but UNconditional PT fails: the covariate-driven trend differs across arms because assignment depends on X. A marginal event study should flag it; the covariate-conditional diagnostic should not

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/PT_conditional_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/PT_conditional_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_PT_conditional.py --reps 100 --jobs 8
    python DGPs/data_creation_PT_conditional.py --all-N        # sweep scenarios only
"""
