#!/usr/bin/env python3
"""DGP: B1_selection_both  (workstream B1, canonical DiD (selection on unobservables)).

unobserved confounder AND covariate-driven assignment: the only setting where conditional parallel trends is violated, so the linearity degree loads through bias

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/B1_selection_both_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/B1_selection_both_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_B1_selection_both.py --reps 100 --jobs 8
    python DGPs/data_creation_B1_selection_both.py --all-N        # sweep scenarios only
"""
