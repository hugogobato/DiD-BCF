#!/usr/bin/env python3
"""DGP: B2_sweep_serial  (workstream B2, canonical DiD (selection on unobservables)).

sample-size sweep under serial correlation

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/B2_sweep_serial_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/B2_sweep_serial_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_B2_sweep_serial.py --reps 100 --jobs 8
    python DGPs/data_creation_B2_sweep_serial.py --all-N        # sweep scenarios only
"""
