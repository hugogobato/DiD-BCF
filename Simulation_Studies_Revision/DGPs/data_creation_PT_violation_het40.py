#!/usr/bin/env python3
"""DGP: PT_violation_het40  (workstream PT, canonical DiD (selection on unobservables)).

heterogeneous violation, slope +/-0.4 per period by X1, zero on average: the marginal event study is powerless by construction and only the subgroup contrast (PRE_SUBC) can detect it

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/PT_violation_het40_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/PT_violation_het40_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_PT_violation_het40.py --reps 100 --jobs 8
    python DGPs/data_creation_PT_violation_het40.py --all-N        # sweep scenarios only
"""
