#!/usr/bin/env python3
"""DGP: D_contamination  (workstream D, staggered adoption (cohort x event-time effects)).

stronger dynamics -> larger TWFE contamination

Writes CSV replications in the **R-benchmark column layout** for every
``linearity_degree`` to
``R_code/D_contamination_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/D_contamination_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_D_contamination.py --reps 100 --jobs 8
    python DGPs/data_creation_D_contamination.py --all-N        # sweep scenarios only
"""
