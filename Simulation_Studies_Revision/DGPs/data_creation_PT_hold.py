#!/usr/bin/env python3
"""DGP: PT_hold  (workstream PT, canonical DiD (selection on unobservables)).
...
``linearity_degree`` to
``R_code/PT_hold_datasets/linearity_degree=<d>/iteration_<rep>.csv`` -- exactly
the files the R estimators in ``R_code/PT_hold_datasets/`` read.  The DiD-BCF and
...
Examples
--------
    python DGPs/data_creation_PT_hold.py --reps 100 --jobs 8
    python DGPs/data_creation_PT_hold.py --all-N        # sweep scenarios only
"""
