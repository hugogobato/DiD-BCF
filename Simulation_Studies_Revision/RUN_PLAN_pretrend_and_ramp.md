...
### Item 3b — the benchmarks now pre-test too

The diagnostic's operating characteristics were measured against **one**
comparator, a TWFE event study. That answers "is it better than the thing
applied work reports" but not "is it better than the estimators we benchmark
against elsewhere in the paper", which is the question a referee asks next.

**What is new.** Every benchmark gets its own pre-trend test, on the same seeded
panels, in the same schema, against the same realised truth — so
`compute_metrics` scores all seven methods on identical definitions. All seven
target the same `Delta(k)`. `Pretrend/README.md` has the full account of what
each test is, which of them are native to their package (Callaway--Sant'Anna and
DoubleML are; synthdid's is the picture its vignette documents, with the
inference constructed; grf-DiD's is the recipe run at an earlier period) and the
two measured caveats worth stating rather than burying — `did2s` is
structurally attenuated because the tested periods sit inside its own first-stage
sample, and synthdid's fitted time weights give the earliest pre-period **zero**
weight.

To make this work three things had to change in the engine, all verified:

* `exports.py` now carries `pt_slope` on the exported panels (a truth column,
  like `CATE`), so the R benchmarks score against the *same realisation* of the
  violation slope that `true_pretrend` uses.
* `metrics.py`'s decision-rule allowlist was an exact-match set of two names, so
  **`joint` and every `PRE_SUBC` `*_any` rule silently aggregated to
  `n_reps = 0, role = undefined`** — including `X1_any`, the row that carries the
  whole heterogeneous-violation argument. It now matches on suffix. (The
  `X1_any` numbers quoted in the earlier revision notes were `X1_slope` values
  that happened to coincide.)
* `run_r_benchmarks.py`, `scaffold_suite.py` and `scaffold_colab_benchmarks.py`
  all excluded workstream PT; they now carry the PT R family and the PT
  notebooks.

**The honest finding to be ready for.** `PRE_SUBC` is *not* unique to DiD-BCF:
a practitioner with a pre-specified binary moderator can sample-split, and
grf-DiD gets the contrast from a single forest with no split at all. On a small
check grf-DiD recovered `X1_slope` essentially unbiasedly where DiD-BCF's own
contrast is ~18% attenuated with 0.81 coverage. What survives is the narrower
claim that DiD-BCF does not need the moderator pre-specified.
...
### The benchmark pre-trend tests (item 3b)

Same local/Colab split as the ramp, and for the same reason: three of the five
are cheap, DoubleML and grf-DiD get notebooks.

```bash
# local: Callaway--Sant'Anna, Gardner, synthdid across all ten PT scenarios
# (200 reps x 2 linearity degrees each; ~0.75 s/rep for the slowest)
python scripts/run_r_benchmarks.py \
    ...
    --scripts did_dr_pretrend did2s_pretrend synthdid_pretrend \
    --reps 200 --batch-size 1
```

Outputs land in `R_code/PT_<scenario>_datasets/summaries_<method>_<scenario>_lin_<d>.csv`,
which `Results/analysis/aggregate_all.py` already globs.

**Colab — 20 notebooks**, `DoubleML_Colab/DoubleML_PT_<scenario>.ipynb` and
`CFFE_Wang_Colab/CFFE_Wang_PT_<scenario>.ipynb`, generated by
`python scripts/scaffold_colab_benchmarks.py --workstream PT`. Unlike the ramp
notebooks these are **self-contained**: the R script is embedded and the seeded
panels are generated inline, so the only thing the clone must supply is the
`PT_*` scenarios in `config.py`. If a run dies at the setup cell with
`KeyError: No experiment named 'PT_hold'`, push the engine and re-run that cell.

| notebook family | cost |
|---|---|
| `CFFE_Wang_PT_*` (10) | seconds per replication — the cheap one |
| `DoubleML_PT_*` (10) | ~14 s/rep locally at `NUM_TREES=500`, i.e. ~1.6 h for 200 reps x 2 degrees; budget 2-4x on a Colab CPU VM, or set `NUM_TREES=100` for ~4x |

Both notebooks' final cell prints the benchmark side by side with DiD-BCF's own
`summaries_pretrend_*` if the clone carries them, so each run is readable on its
own without waiting for the full aggregation.
