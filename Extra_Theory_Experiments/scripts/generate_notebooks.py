#!/usr/bin/env python3
"""Generate validation, 48-way family shards, and two worst-case BCF pilots."""
...
OUT = str(TARGET / "Extra_Theory_Experiments" / "results" /
          "extra_theory_{notebook_family}_{output_suffix}")
...
write_provenance(os.path.join(OUT, "provenance.json"), tasks=tasks,
                 ...
                 config={{"family": FAMILY, "reps": REPS, "shard": SHARD_ID,
                         "n_shards": N_SHARDS, "K": 2, "workers": 1,
                         "pilot": {pilot_value}}})
with open(os.path.join(OUT, "README_run.txt"), "w", encoding="utf-8") as handle:
    handle.write("Family=" + FAMILY + "; shard=" + str(SHARD_ID) + "/" + str(N_SHARDS) + "; reps=" + str(REPS) +
                 "; smoke=" + str(SMOKE) + "\\n")
output_file = OUT + ".zip"
with zipfile.ZipFile(output_file, "w", compression=zipfile.ZIP_DEFLATED) as archive:
    ...
    for family in ("correction_audit", "information_ablation"):
        for shard in range(48):
            ...
            (out / name).write_text(
                ...
                encoding="utf-8")
    for pilot_name in ("correction", "information"):
