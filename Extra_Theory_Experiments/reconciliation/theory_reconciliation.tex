...
48 of 48 information shards) and the published revision summaries, through a single
reproducible script.
\end{abstract}
...
Both families are now complete. The correction family has all $48$ shards and the
information family has all $48$ shards, so every cell has $100$ replications; the
$2{,}400$ oracle requests on the production DGPs remain explicitly ``unavailable'' by
...
With all correction shards in hand the account is complete. The published plain and
...
pilots, and by construction on the oracle design. The theory-aligned construction fixes
...
posterior rate, they do not have degree $d=3$ in the correction grid, they use finite
...
All calibration cells now have $100$ replications, and the published and new runs use
...
\clearpage
\input{tables/tab_repro_baseline}
\input{tables/tab_repro_serial}
\input{tables/tab_repro_att}
\input{tables/tab_repro_sweep}
\input{tables/tab_oracle}
\input{tables/tab_bench_gatt}
\input{tables/tab_bench_gatt_800}
\input{tables/tab_bench_att}
\input{tables/tab_info_gatt}
\input{tables/tab_info_att}
\input{tables/tab_null}
